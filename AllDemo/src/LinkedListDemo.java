...
//        singleLinkedList.
        ...
        singleLinkedList.reverseSingleLinkedList(singleLinkedList);
...
class SingleLinkedList {
    ...
    //反转单链表
    public void reverseSingleLinkedList(SingleLinkedList singleLinkedList) {
        if (singleLinkedList.head.next == null || singleLinkedList.head.next.next == null) {
            System.out.println("链表长度小于2，不能进行反转");
        }
        HeroNode oldElement = singleLinkedList.head.next;
        HeroNode newHeard = new HeroNode(0, "", "");
        HeroNode tempNode = null;
        while (oldElement != null) {
            tempNode = oldElement.next;
            oldElement.next = newHeard.next;
            newHeard.next = oldElement;
            oldElement = tempNode;

        }
        singleLinkedList.head.next = newHeard.next;
